...
def test_e131_round_trip():
    ...
    packet = protocol.build_e131(
        7, payload, sequence=5, cid=bytes(range(16)),
        source_name=b"proxy".ljust(64, b"\x00"), priority=120)
...
def test_e131_pdu_lengths_match_the_standard():
    ...
    packet = protocol.build_e131(1, bytes(512), sequence=1, cid=bytes(16),
                                 source_name=bytes(64))
...
def test_e131_ignores_preview_and_non_zero_start_code():
    packet = bytearray(protocol.build_e131(1, bytes(30), sequence=1, cid=bytes(16),
                                           source_name=bytes(64)))
