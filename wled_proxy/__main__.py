...
from . import __version__, app, config as config_module
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--log-level", choices=config_module.LOG_LEVELS,
...
def print_check(config: config_module.Config) -> None:
    ...
    print(f"strip  : {strip.led_count} pixels, {strip.format.upper()}, "
          f"{strip.channels} channels")
    enabled_inputs = [name for name, section in (("ddp", config.inputs.ddp),
                                                 ("artnet", config.inputs.artnet),
                                                 ("e131", config.inputs.e131))
                      if section.enabled]
    print(f"inputs : {', '.join(enabled_inputs) or 'none'}")
    print(f"targets: {len(targets)} enabled, "
          f"{len(config.targets) - len(targets)} disabled")
    for target in targets:
        print(f"  {target.name:<20} {target.start:>6}-{target.end - 1:<6} "
