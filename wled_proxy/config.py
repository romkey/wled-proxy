...
from .protocol import (ARTNET_CHANNELS_RGB, ARTNET_CHANNELS_RGBW, ARTNET_PORT,
                       DDP_PORT, E131_DEFAULT_PRIORITY, E131_PORT)
...
class ArtnetInputConfig:
    ...
    def __post_init__(self):
        self.channels_per_universe = (self.channels_per_universe
                                      or _default_universe_size(self.format))
...
def _parse_inputs(section: Section, strip: StripConfig) -> InputsConfig:
    ...
    inputs.artnet = ArtnetInputConfig(
        ...
        channels_per_universe=art.integer(
            "channels_per_universe", _default_universe_size(art_format), minimum=1, maximum=512),
    ...
    inputs.e131 = E131InputConfig(
        ...
        channels_per_universe=e131.integer(
            "channels_per_universe", _default_universe_size(e131_format), minimum=1, maximum=512),
...
def _parse_targets(entries: list, defaults: dict, strip: StripConfig) -> list[TargetConfig]:
    ...
    for index, entry in enumerate(entries):
        ...
        target = TargetConfig(
            ...
            universe=section.integer("universe", 0 if proto == "artnet" else 1,
                                     minimum=0, maximum=63999),
            channels_per_universe=section.integer(
                "channels_per_universe", _default_universe_size(fmt), minimum=1, maximum=512),
            priority=section.integer("priority", E131_DEFAULT_PRIORITY,
        ...
        if target.protocol in ("artnet", "e131"):
            if target.channels_per_universe % target.channels_per_pixel:
                raise ConfigError(
