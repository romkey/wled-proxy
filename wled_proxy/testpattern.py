...
def _hsv(hue: float) -> bytes:
    r, g, b = (max(0.0, min(1.0, 1.5 - abs(((hue + shift) % 1.0) * 6.0 - 3.0)))
...
def send_ddp(sock, address, payload: bytes, rgbw: bool, sequence: int) -> int:
    ...
    while position < total:
        ...
        position += len(chunk)
        sock.sendto(protocol.build_ddp(position - len(chunk), chunk, rgbw=rgbw,
                                       sequence=sequence, push=position >= total), address)
    return sequence
...
def send_artnet(sock, address, payload: bytes, size: int, universe: int, sequence: int) -> int:
    ...
    for position in range(0, len(payload), size):
        sock.sendto(protocol.build_artnet_dmx(universe, payload[position:position + size],
                                              sequence=sequence), address)
        universe += 1
    return sequence


def send_e131(sock, address, payload: bytes, size: int, universe: int,
              sequences: dict, cid: bytes, source: bytes) -> None:
    for position in range(0, len(payload), size):
        sequences[universe] = (sequences.get(universe, 0) + 1) & 0xFF
        sock.sendto(protocol.build_e131(universe, payload[position:position + size],
                                        sequence=sequences[universe], cid=cid,
                                        source_name=source), address)
        universe += 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    universe = args.universe if args.universe is not None else (0 if args.protocol == "artnet" else 1)
    universe_size = protocol.ARTNET_CHANNELS_RGBW if args.format == "rgbw" else protocol.ARTNET_CHANNELS_RGB
    try:
        color = tuple(int(v) & 0xFF for v in args.color.split(","))
        if len(color) != 3:
            raise ValueError
    except ValueError:
        print("--color needs three comma separated numbers, e.g. 255,80,0", file=sys.stderr)
        return 2
    ...
    print(f"sending {args.pattern} to {args.host}:{port} over {args.protocol.upper()} "
    ...
    try:
        while True:
            ...
            if args.protocol == "ddp":
                ...
                send_e131(sock, address, frame, universe_size, universe, sequences, cid, source)
